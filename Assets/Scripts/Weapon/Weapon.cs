...
public class Weapon : MonoBehaviour
{
	...
	[HideInInspector]
	public CharacterNetwork controller;
	...
	[HideInInspector]
	public RectTransform PdynamicReticle;
	...
	private void Start()
	{
		...
		controller = FindObjectOfType<CharacterNetwork>();
	...
	void Update()
	{
		...
		if (InputManager.instance.GetFire() && !PlayerStats.isPlayerDead && weaponType != WeaponType.Pistol && !InventoryManager.showInventory && fireMode == FireMode.automatic)  //Statement to restrict auto-fire for pistol weapon type. Riffle and others are automatic
		{
			Fire();
		}
		else if (InputManager.instance.GetFire() && !PlayerStats.isPlayerDead && (weaponType == WeaponType.Pistol || fireMode == FireMode.single) && !InventoryManager.showInventory)
		{
			Fire();
		}
		...
		if (weaponType != WeaponType.Melee && weaponType != WeaponType.Grenade)
		{
			...
			if (InputManager.instance.GetReload() || currentAmmo < 0)
			{
